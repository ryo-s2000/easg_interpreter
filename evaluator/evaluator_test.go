...
func TestArrayLiteral(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Fatalf("array has not 3. got=%d", len(result.Elements))
	}
...
func TestHashLiterrals(t *testing.T) {
	input := `let two = "two";
	{
		"one": 10-9,
		"two": 1 + 1,
	...
	if !ok {
		t.Fatalf("Eval did not return true")
	}
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash has wrong Pairs len")
	}
